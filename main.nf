...
workflow {
    ...
    tomboresquiggle_n_16s_indexed = Channel.from(tomboresquiggle_n_16s.out.resquiggledone.collect().withIndex().collect {it,i -> [i,it]})
    tomboresquiggle_i_16s_indexed = Channel.from(tomboresquiggle_i_16s.out.resquiggledone.collect().withIndex().collect {it,i -> [i,it]})
